...
public class Chasing : MonoBehaviourPunCallbacks {
	...
	PlayerScript targetNetworkPlayer;
	...
	public void SetTarget(GameObject player) {
		...
		if(player == null) {
			shouldUpdate = false;
			photonView.RPC("RPCSyncTarget", Photon.Pun.RpcTarget.Others, "");
		}
		else {
			targetNetworkPlayer = target.GetComponent<PlayerScript>();
			photonView.RPC("RPCSyncTarget", Photon.Pun.RpcTarget.Others, player.GetComponent<PlayerScript>().playerName);
		}
	}
	...
	void RPCSyncTarget(string playerName) {
		...
		foreach(GameObject player in players) {
			if(player.GetComponent<PlayerScript>().playerName == playerName) {
				target = player;
				targetNetworkPlayer = target.GetComponent<PlayerScript>();
				break;
			}
	...
	public void SetDamage(float newDamage) {
		damage = newDamage;
		photonView.RPC("RPCSetDamage", Photon.Pun.RpcTarget.Others, newDamage);
	}
	...
	public void SetSpeed(float newSpeed, float newAngularSpeed) {
		...
		photonView.RPC("RPCSetSpeed", Photon.Pun.RpcTarget.Others, newSpeed, newAngularSpeed);
	}
